(() => {
      ...
      var EmojiApi2 = class {
        async convertToEmoji(text, callback) {
          return axios.post("https://makers-emojify.herokuapp.com/", {
            "text": text
          }).then((response) => callback(response.data.emojified_text)).catch((e) => console.log(e));
        }
      };
  ...
  var view = new NotesView(model, api, emojiApi);
  api.loadNotes((notes) => {
    console.log(notes);
    model.setNotes(notes);
    view.displayNotes();
  });
  var result = emojiApi.convertToEmoji("Hello, :earth_africa:", (res) => res);
  console.log(result);
